...
local bNextSeasonChangeTurnMissing;
...
-- Severity							 	 	 1		 2		 3		 4	   	 5
-- UNITS
local kSummerExperienceEffects_UI = 	{	 0, 	 0, 	 0, 	 0, 	 0	};
...
-- Severity							 	 	 1		 2		 3		 4	   	 5
-- UNITS
local kSpringExperienceEffects_UI = 	{	 0, 	 0, 	 0,  	20, 	40	};
...
function CheckGameLoad()
	...
	if Game:GetProperty("iSeasonCounter") == nil then
		...
		iNextSeasonOffset = math.random(-kMaxSeasonOffset_UI, kMaxSeasonOffset_UI);
		local iCurrentTurn = kStartTurn;
		iNextSeasonChangeTurn = iCurrentTurn + (kSeasonLength * 2) + iNextSeasonOffset;
	...
	-- old game loaded
		...
		-- these variable names were not changed in V0.2
		...
		-- new in V0.2
		if Game:GetProperty("iNextSeasonChangeTurn") == nil then
			bNextSeasonChangeTurnMissing = true;
		else
			bNextSeasonChangeTurnMissing = false;
			iNextSeasonChangeTurn = Game:GetProperty("iNextSeasonChangeTurn");
		end
		
		-- these variables had their names changed in V0.2, forcing us to check if the game has them stored in its save file
		if Game:GetProperty("iThisSeasonChangeTurn") == nil then
			-- old variable name
			iThisSeasonChangeTurn = Game:GetProperty("iSeasonChangeTurn");
		else
			iThisSeasonChangeTurn = Game:GetProperty("iThisSeasonChangeTurn");
		end
		if Game:GetProperty("iNextSeasonSeverity_UI") == nil then
			iNextSeasonSeverity_UI = Game:GetProperty("iNewSeasonSeverity_UI");
		else
			iNextSeasonSeverity_UI = Game:GetProperty("iNextSeasonSeverity_UI");
		end
		if Game:GetProperty("iThisSeasonSeverity") == nil then
			iThisSeasonSeverity = Game:GetProperty("iLastSeasonSeverity");
		else
			iThisSeasonSeverity = Game:GetProperty("iThisSeasonSeverity");
		end

		if Game:GetProperty("sThisSeason") == nil then
			sThisSeason = Game:GetProperty("sCurrentSeason");
		else
			sThisSeason = Game:GetProperty("sThisSeason");
		end
		if Game:GetProperty("iNextSeasonSeverity_UI") == nil then
			sNextSeason_UI = Game:GetProperty("iNewSeasonSeverity_UI");
		else
...
function FourSeasons()	
	...
	elseif iCurrentTurn == iThisSeasonChangeTurn and bSeasonChange_UI then
		if bNextSeasonChangeTurnMissing then
			iNextSeasonOffset = math.random(-kMaxSeasonOffset_UI, kMaxSeasonOffset_UI);
			iThisSeasonChangeTurn = iThisSeasonChangeTurn + kSeasonLength + iNextSeasonOffset;
		else
