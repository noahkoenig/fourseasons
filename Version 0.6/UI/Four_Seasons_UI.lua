...
local kStartIntensity_UI = Game:GetProperty("kStartIntensity_UI");
...
function CheckGameLoad()
	...
	if iCurrentTurn == kStartTurn then
		if Controls.SummerButton ~= nil then
			...
			local sYourIntensityBias = table.concat({Locale.Lookup("LOC_FOUR_SEASONS_START_INTENSITY_BIAS_TEXT"), " ", tostring(kIntensityBias)});
			...
			sToolTip = table.concat({sWelcome, "[NEWLINE][NEWLINE]", sYourSettings, "[NEWLINE][NEWLINE]", sYourSeasonLength, "[NEWLINE]", sYourIntensityBias, "[NEWLINE]", sChangeTime, "[NEWLINE][NEWLINE]", sSeeCivilopedia});
...
function GetToolTipString(bGameStart)
	if bGameStart then
		...
		iNextSeasonIntensity_UI = kStartIntensity_UI;
	else
		sNextSeason_UI = Game:GetProperty("sNextSeason_UI");
	...
	local sIntensityIcon;
	if iNextSeasonIntensity_UI == 6 then
		sIntensityIcon = " [ICON_Exclamation]";
	else
		sIntensityIcon = "";
	end
	local sIntensityInfo = table.concat({Locale.Lookup("LOC_FOUR_SEASONS_INTENSITY_TEXT"), " ", tostring(iNextSeasonIntensity_UI), sIntensityIcon});
	
	local iSeasonIndex = GetSeasonIndex(sNextSeason_UI);

	local sUnitEffects = GetUnitEffects(iSeasonIndex, iNextSeasonIntensity_UI);
	local sCityEffects = GetCityEffects(iSeasonIndex, iNextSeasonIntensity_UI);
	local sWarEffects = GetWarEffects(iSeasonIndex, iNextSeasonIntensity_UI);	
	local sAllEffects = GetAllEffects(sUnitEffects, sCityEffects, sWarEffects);

	local sNextSeasonInfo = GetNextSeasonInfo(sNextSeason_UI, iNextSeasonIntensity_UI);
		
	-- for some reason a concatenation with .. isn't allowed in UI scripts		
	return table.concat({sThisSeason, " - ", sIntensityInfo, "[NEWLINE][NEWLINE]", sAllEffects, "[NEWLINE]", sNextSeasonInfo});
end
...
-- ===========================================================================
function GetUnitEffects(iSeasonIndex, iIntensity)
	local sExperienceChange = GetExperienceChange(iSeasonIndex, iIntensity);
	local sHealingChange = GetHealingChange(iSeasonIndex, iIntensity);
	local sMaintenanceChange = GetMaintenanceChange(iSeasonIndex, iIntensity);
	local sMovementChange = GetMovementChange(iSeasonIndex, iIntensity);
	local sSeaMovementChange = GetSeaMovementChange(iSeasonIndex, iIntensity);
...
-- ===========================================================================
function GetCityEffects(iSeasonIndex, iIntensity)
	local sFoodChange = GetFoodChange(iSeasonIndex, iIntensity);
	local sProductionChange = GetProductionChange(iSeasonIndex, iIntensity);
...
-- ===========================================================================
function GetWarEffects(iSeasonIndex, iIntensity)
	local sWarWearinessChange = GetWearinessChange(iSeasonIndex, iIntensity);
...
-- ===========================================================================
function GetNextSeasonInfo(sSeason, iIntensity)
...
-- ===========================================================================
-- Returns experience change as a string based on current season and intensity.
-- ===========================================================================
function GetExperienceChange(iSeasonIndex, iIntensity)
	local iExperienceValue = kExperienceEffects_UI[iSeasonIndex][iIntensity];
...
-- ===========================================================================
-- Returns healing change as a string based on current season and intensity.
-- ===========================================================================
function GetHealingChange(iSeasonIndex, iIntensity)
	local iHealingValue = kHealingEffects_UI[iSeasonIndex][iIntensity];
...
-- ===========================================================================
-- Returns maintenance change as a string based on current season and intensity.
-- ===========================================================================
function GetMaintenanceChange(iSeasonIndex, iIntensity)
	local iMaintenanceValue = kMaintenanceEffects_UI[iSeasonIndex][iIntensity];
...
-- ===========================================================================
-- Returns movement change as a string based on current season and intensity.
-- ===========================================================================
function GetMovementChange(iSeasonIndex, iIntensity)
	local iMovementValue = kMovementEffects_UI[iSeasonIndex][iIntensity];
...
-- ===========================================================================
-- Returns sea movement change as a string based on current season and intensity.
-- ===========================================================================
function GetSeaMovementChange(iSeasonIndex, iIntensity)
	local iSeaMovementValue = kSeaMovementEffects_UI[iSeasonIndex][iIntensity];
...
-- ===========================================================================
-- Returns food yield change as a string based on current season and intensity.
-- ===========================================================================
function GetFoodChange(iSeasonIndex, iIntensity)
	local iFoodValue = kFoodEffects_UI[iSeasonIndex][iIntensity];
...
-- Returns food yield change as a string based on current season and intensity.
-- ===========================================================================
function GetProductionChange(iSeasonIndex, iIntensity)
	local iProductionValue = kProductionEffects_UI[iSeasonIndex][iIntensity];
...
-- ===========================================================================
-- Returns war weariness change as a string based on current season and intensity.
-- ===========================================================================
function GetWearinessChange(iSeasonIndex, iIntensity)
	local iWearinessValue = kWearinessEffects_UI[iSeasonIndex][iIntensity];
