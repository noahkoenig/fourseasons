...
-- If you change this, make sure to apply the Summer modifiers in CheckGameLoad().
...
-- These tables provide the effect values. You only have to edit the values here and both Gameplay and UI adjust automatically.
...
-- Severity							 	 	 1		 2		 3		 4	   	 5
-- UNITS
local kAutumnExperienceEffects_UI = 	{	 0, 	20, 	40, 	40, 	60	};
local kAutumnHealingEffects_UI = 		{	 0,      0, 	-5, 	-5,    -10	};
...
local kWinterHealingEffects_UI = 		{	 0, 	-5, 	-5,    -10,    -10,	  -15	};
local kWinterMaintenanceEffects_UI = 	{	 1,      1, 	 2, 	 3, 	 4, 	5	};
local kWinterMovementEffects_UI = 		{ 	 0, 	-1, 	-1, 	-1, 	-2,	   -2	};
local kWinterSeaMovementEffects_UI = 	{ 	 0, 	 0, 	 0,  	-1, 	-1,	   -1	};
-- CITIES
...
local kSpringMovementEffects_UI = 		{	 1, 	 0, 	 0, 	 0, 	-1	};
local kSpringSeaMovementEffects_UI = 	{	 1, 	 0, 	 0, 	 0, 	 0	};
-- CITIES
local kSpringFoodEffects_UI = 			{	 0, 	 0, 	-1, 	-1, 	-1	};
local kSpringProductionEffects_UI = 	{	 0, 	 0, 	 0, 	 0, 	-1	};
-- WARS
...
function GetNewSeverity(sSeason, iSeverity)
	...
	-- if autumn had a severity of 5, there is a 10% chance that the winter will have a severity 6
		if sSeason == "WINTER" and iSeverity == 5 then
			tDistribution = {1, 2, 3, 3, 4, 4, 4, 5, 5, 6};
		else
			if iSeverity == 1 then
				tDistribution = {1, 1, 1, 2, 2, 2, 3, 3, 4, 5};
			...
			-- last severity was 5 or 6 
				...
				tDistribution = {1, 2, 3, 3, 4, 4, 4, 5, 5, 5};
			end
		end
	end
...
-- maybe (?) a way to access individual plots. at least there was no error in the console.
--[[local pCityPlots = pCity:GetOwnedPlots();
for _, pPlot : object in pairs(pCityPlots) do
	if pPlot ~= nil then
		pPlot:AttachModifierByID("FOUR_SEASONS_CITY_PLOT_YIELDS_ADJUST_FOOD_YIELD_BY_1");
	end
end]]--
...
function AttachSummerModifiers(iSeverity, bUndo)
	...
	for _, pPlayer in pairs(pPlayers) do
		...
		-- undo movement change for sea units and apply separate movement change instead
